...
- **Coord doc (append-only joint log):** `~/Documents/gosteady-portal/docs/firmware-coordination/2026-04-17-cloud-contracts.md` _(moved out of iCloud 2026-05-23 to fix tsc/npm slowness; iCloud copy at the old path may persist for a short confidence-window before deletion)_
  ...
  - **Latest entry: §C33 (2026-05-26)** — Alert recurrence policy landed: continuous-condition alerts (`battery_critical/low`, `signal_lost/weak`, `device_offline/silent`) now suppress new rows while same-type alert is open, and auto-ack via `system:condition_cleared` when the underlying condition clears. New `Patient.openAlerts: Map<alertType, {sk, openedAt}>` slot on Patient row; new `_shared/open_alerts.py` helper; new `alert.auto_acknowledged` audit event. Migration script bulk-acked 71 pre-policy duplicate alerts across 9 dev patients (49 of those on `pt_bench_98` — the 45-alert pileup the policy was designed to prevent). Bench-validated all five state transitions (recovery, tier change, suppression, escalation). Daily-cadence rules (`no_activity_today`, `below_typical_activity`, `declining_trend`) NOT under this policy — each day's row is the historical record. **Zero firmware-facing impact** — cloud state-machine fix; MQTT contracts unchanged. Prior 2026-05-26 work: §C32 L5 throttle; §C31 TTL caching; §C30 L6 notification swap; §C29 PollingController; §C28.5 CR-1 deploy. Next portal subset: 2B-FAC-W (facility writes — alert ack + Add/Edit/Discharge/Pause/CareNote)

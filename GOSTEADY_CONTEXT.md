...
- **Activity-uplink stale `stop_done_sem` race fix + visibility patch (2026-05-05, commits `fede856` + `4d62fd5`).** Discovered when the cloud team's Phase 1.6 Per-Device Detail dashboard went live and showed zero activity-DDB rows for the bench unit despite obvious walks happening over the previous 5 days. Heartbeat + snippet uploads were working (proves cellular + cloud-uplink path was healthy at the IoT-topic level), but every activity-uplink attempt was being silently skipped. **Diagnostic patch first (`fede856`):** added writer-thread `LOG_INF`s at start handshake / first-sample seed / stop branch to trace `s_pipeline_seeded` transitions, plus a boot-time `gs_pipeline_init` success log, plus stopped discarding the activity worker's `connect_publish_disconnect` return value (was `(void)`; now logs WRN with rc on failure + INF on success). Bonus: closed the stale `TODO(M12)` on `rewrite_header` that was leaving `session_end_utc_ms = 0` even though M12.1a had cellular UTC available — added `gosteady_cellular_get_network_time_unix_ms()` helper using Zephyr's `timeutil_timegm64` and use it for both `session_start_utc_ms` and `session_end_utc_ms` in the .dat header. **Smoking gun:** synthetic 8 s session showed `pipeline_seeded=1` cleanly, ALGO_V1A logged, activity uplink fired. But a 44 s real walk session (`203c9073`) reported `pipeline_seeded=1` in the writer's diagnostic log 65 ms AFTER session_stop body had already emitted the `no outputs (pipeline not seeded)` WARN. **Root cause** (commit `4d62fd5`): the previous session's stop branch hit the `K_SECONDS(5)` timeout on `k_sem_take(&stop_done_sem, ...)` because `rewrite_header + fs_close + snippet_capture_finish` exceeds 5 s under LittleFS GC pressure on a near-full sessions partition (84 % utilization, 87 .dat files accumulated across 4 firmware versions). When the writer eventually finished its slow path and called `k_sem_give`, the count=1 sat on the sem. Next session's `k_sem_take` returned instantly (consuming the stale count), `session_stop` body read `s_pipeline_outputs_valid` before the writer had a chance to set it true → WARN → activity uplink path silently skipped. **Fix:** `k_sem_reset(&stop_done_sem)` immediately before raising stop_signal in `gosteady_session_stop()`, plus bumped the take timeout 5 s → 15 s for headroom. **Verified end-to-end against 5 real walks post-fix** (35 / 40 / 10 / 81 / 60 steps; M9 algo outputs all populated; one walk classified outdoor at R=0.4033 — interesting v1.5 retrain data point). Cloud-side companion fix in `gosteady-portal` commit `ec258f5` (activity-processor audit log now includes full firmware-derived `after` block; per-device dashboard query field names aligned with audit log shape). All four cloud uplinks now bench-validated with real-walk data: heartbeat / activity / cmd ingest / snippet. **Bench operational note:** before site-survey ship, run `tools/cleanup_device.py --all --yes` to drop the partition utilization back below GC-stall territory; long-term fix is auto-prune-on-publish-success once activity uplink ack is reliably tracked. **Outcome:** firmware is now ready for M14.5 site-survey shakedown.
- **Auto-prune-on-publish-success + graceful ENOSPC handling (2026-05-06, commit `bde3434`).** Architectural fix for the partition-fill failure mode that yesterday's `cleanup_device.py --all --yes` only papered over. Two halves: (1) **auto-prune** — `gosteady_activity` struct gains a `session_uuid[37]` field; cloud.c's activity worker calls a new `gosteady_session_prune(uuid)` (defined in session.h, validated 36-char hyphenated UUID before pathing into SESSION_DIR for `fs_unlink`) immediately after PUBACK on the activity uplink. The .dat file becomes redundant once algo outputs are in cloud's Activity Series DDB and the snippet on the snippet partition is the v1.5 retrain corpus. Bounds growth of the sessions partition. (2) **Graceful ENOSPC** — new `s_writer_flash_full` flag in session.c set by `writer_flush` on -ENOSPC from `fs_write` (logged ONCE per session, not the EAGAIN flood seen yesterday); new `gosteady_session_flash_full()` accessor; main.c heartbeat tick polls it alongside the existing stationary-samples auto-stop so the session closes cleanly when the writer can't drain (the activity-uplink path runs against valid in-memory algo outputs, then auto-prune frees a slot for the next session). main.c sampler also rate-limits the `session_append failed (-35)` WRN to 1-per-100-failures so log flooding doesn't bury other diagnostics. **Validated against bench unit GS9999999999** post-flash: synthetic session 56c8037b enqueued, dequeued, PUBACK'd, then `auto-prune: deleted /lfs/sessions/56c8037b-...dat after activity PUBACK` fired; LIST on device confirms the .dat is gone. Cloud-side row at 2026-05-06T17:42:51Z. **Side-quest along the way:** during reflash, my flash attempts kept hitting `Eraseprotect is enabled and readback protection setting is ALL` on what appeared to be the nRF9151. Turned out **SW2 had been bumped to the nRF53 position** at some point during the day; nrfjprog was operating on the nRF5340 bridge chip with `-f NRF91` family flag, corrupting the bridge's protection state. Recovery: `nrfjprog -f NRF53 --coprocessor CP_NETWORK --recover` (per Programmer's hint that recovering nRF53 network core cascade-recovers app core) → reflash bridge from `build_bridge/merged.hex` → toggle SW2 back to nRF91 → confirm the actual nRF9151 was running this morning's working firmware untouched (its CPU registers read out cleanly, certs and `/lfs/activation.bin` intact). **Operational lesson for M14.5:** always verify SW2 position before any nrfjprog recovery commands — a wrong-chip recover with the wrong family flag can cascade-corrupt the unintended chip.
...
| 2026-04-29 | Firmware | §F7.1–§F7.7 | **M12.1e.2 milestone-complete.** Activate cmd flow works end-to-end: cloud publish → broker queue (CLEAN_SESSION=n) → device CONNECT delivers queued cmd → JSON parses → `activation_apply` persists to `/lfs/activation.bin` → `Shadow.reported.activated_at` written → reboot survives state. New `src/activation.{h,c}` owns persistence; `cloud.c` registers `gs/{serial}/cmd` via `aws_iot_application_topics_set` + dispatches activate cmds; `session.c` adds a `-EACCES` pre-activation gate gated on `CONFIG_GOSTEADY_FIELD_MODE`. Two design changes worth flagging: `CONFIG_MQTT_CLEAN_SESSION` flipped y→n (broker-side queueing of QoS 1 cmds across hourly heartbeats); 1.5 s linger after PUBACK before disconnect (gives queued cmds time to flow in). Shadow re-check on every wake INTENTIONALLY DEFERRED per §C6.3 (cloud device-shadow-handler is Phase 2A; per-wake re-check has no observable effect until that ships). Blue LED slow-blink also deferred to M14.5 polish. |
| 2026-04-29 | Firmware | §F8.1–§F8.7 | **M12.1f milestone-complete — closes the M12.1 cellular work-block.** Snippet capture + opportunistic upload working end-to-end against `GS9999999999`: 30 s window of raw 100 Hz BMI270 samples → §F.4 binary format → §F.3 outer framing → publish to `gs/{serial}/snippet` after heartbeat PUBACK / before disconnect → cloud IoT Rule + parser Lambda → `s3://gosteady-dev-snippets/GS9999999999/2026-04-29/<snippet_id>.bin` (43,122 B verified bit-exact). New `src/snippet.{h,c}` owns capture (per-session, 64-sample in-RAM batch flushes to `/snippets/<uuid>.bin`) + upload (heartbeat-piggyback, FIFO oldest-first, `.up` marker = uploaded). Three bugs surfaced + fixed during validation (§F8.4): per-sample fs_write blew the writer-thread stack (fix: 64-sample batch + writer_stack 3072→4096); two 84 KB BSS buffers RAM-overflowed (fix: single shared buffer); orphan `.bin` from previous-boot fault loops upload forever (fix: best-effort fs_unlink when sidecar missing). All four firmware feature milestones now bench-validated (heartbeat / activity / cmd ingest / snippet). v1 deferrals listed in §F8.5: scheduled cadence, anomaly trigger, 8/day cap, battery floor, 14-day stale cutoff. |
...
### M12.1e.2 — Pre-activation gate + activate cmd ingest — DONE 2026-04-29 (commit `a45a02d`)

Activate cmd flow works end-to-end against `GS9999999999`. Cloud publishes to `gs/{serial}/cmd`, AWS IoT broker queues with CLEAN_SESSION=n persistent-session semantics, device CONNECT on next heartbeat delivers the queued cmd, firmware JSON-parses (`{cmd, cmd_id, ts, session_id}` schema per coord §1.2), `gosteady_activation_apply` persists to `/lfs/activation.bin`, `gosteady_cloud_set_last_cmd_id` sets the heartbeat-extras echo, `write_reported_activated_at` writes the Shadow ack. Verified via `aws iot-data get-thing-shadow`: `state.reported.activated_at = "2026-04-29T22:35:00Z"` ✓. Reboot persistence confirmed — `/lfs/activation.bin` survives SoC reset and the next boot's `gs_activation: activated: at=... cmd_id=...` log line shows the state correctly loaded. Pre-activation gate in `gosteady_session_start` returns `-EACCES` when not activated; gated on `CONFIG_GOSTEADY_FIELD_MODE` so bench builds keep working unchanged. Two deferrals per §F7.5: Shadow re-check on every cellular wake (no observable effect until cloud's Phase 2A `device-shadow-handler` Lambda ships per §C6.3); blue LED slow-blink (pure visual polish; M14.5).

**Two design changes load-bearing for production behavior:**
- **`CONFIG_MQTT_CLEAN_SESSION=n`** in both `prj_cloud.conf` and `prj_field.conf`. Original `=y` setting silently dropped any cloud-published activate cmd while the device was between hourly heartbeats. With persistent sessions + the same client_id, AWS IoT holds QoS 1 messages per-client until next CONNECT. First connect after the change shows `persistent_session=0` (broker bootstraps); subsequent connects show `persistent_session=1` and queued cmds flow on resume.
- **1.5 s linger after PUBACK** before `aws_iot_disconnect` in `connect_publish_disconnect`. Gives queued app-topic deliveries time to arrive and the synchronous event handler to dispatch `handle_activate_cmd` before tear-down. Cellular RRC stays connected ~1.5 s longer per heartbeat = ~0.5 mC additional cost; revisit after M14.5 if battery numbers warrant.

### M12.1f — Snippet capture + opportunistic upload — DONE 2026-04-29 (commit `de89df4`)

Snippet capture + upload working end-to-end. Per-session 30 s window of raw 100 Hz BMI270 samples lands at `s3://gosteady-dev-snippets/GS9999999999/2026-04-29/<snippet_id>.bin` (43,122 B verified bit-exact). New `src/snippet.{h,c}` owns the full path: capture into a 64-sample in-RAM batch flushed to `/snippets/<uuid>.bin` on the dedicated `snippet_storage` LittleFS instance, sidecar `<uuid>.json` for the JSON header, `<uuid>.up` zero-byte marker = uploaded. Upload happens on the heartbeat thread AFTER PUBACK / BEFORE disconnect — single 84 KB BSS buffer holds the framed payload (4-byte BE prefix + JSON header + 16-byte binary header + N × 28 B samples). Cloud team's IoT Rule + parser Lambda processed the §F.3 framing first try.

**Three bugs surfaced + fixed during M12.1f validation (per §F8.4):**
- Per-sample `fs_write` from the writer thread overflowed its 3072 B stack on auto-stop's session_stop sequence (3000 writes/window deepens the call stack with LittleFS metadata churn). Fix: 64-sample in-RAM batch + writer_stack bumped 3072 → 4096.
- Two 84 KB BSS buffers (one for binary read, one for outer framing) overflowed the RAM region by 80 KB. Fix: single shared 84 KB buffer, read .json + .bin directly into their slots in the outer-framing layout.
- Orphan `.bin` from a previous-boot HardFault (capture started but capture_finish never ran) caused find_pending_uuid to loop forever on the missing sidecar. Fix: best-effort `fs_unlink` of the orphan `.bin` when sidecar is missing.

**v1 deferrals (per §F8.5):**
- Scheduled trigger (every 6 hr per M10.5) — v1 always-arms every session.
- Anomaly trigger (R outlier / σ outlier / >4 g) — v1 doesn't classify.
- 8 snippets/day cap — v1 retain-forever (16 MB partition holds 280-1100 typical snippets).
- `battery_pct ≥ 30 %` gate — v1 implicit (heartbeat already validated battery before drain runs).
- 14-day stale cutoff + 90 % full rotation — v1 retain-forever; rotation only kicks in on `fs_write -ENOSPC`.
- MQTT 5 user properties — confirmed unsupported in NCS 3.2.4 (Zephyr MQTT 3.1.1 only). JSON-header fallback per §F.3 is the v1 path; matches what cloud team's IoT Rule + Lambda expects.

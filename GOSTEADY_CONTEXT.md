...
Milestones 1–5 are done. Next is **Milestone 6 — BLE control**: Nordic UART Service (NUS) over BLE on the nRF5340, routed through the connectivity bridge, so a phone or laptop can drive `start_session` / `stop_session` remotely with the full 12-field PRE-WALK metadata payload instead of the hard-coded bench defaults. The line-protocol we built for uart1 in M5 is essentially the same shape — extending or re-using its command vocabulary for the BLE characteristic writes is the natural move. After that, M7 Python CLI wraps both channels (BLE control + uart1 pull) behind a single host tool. 

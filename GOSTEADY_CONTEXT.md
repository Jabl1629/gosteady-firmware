...
Firmware version: **`0.11.0-wipe-cmd`** (as of 2026-05-17 — adds `wipe` downlink-cmd handler for the AA-battery-recycle design that supersedes ARCH DL6's charger-gated reset model; commit `5d73684`; **code committed but NOT yet flashed to GS9999999998** — flash deferred to next bench session per coord §C21). Previous versions: `0.10.0-at-timeout` (bounded-timeout AT wrapper in `cellular.c`, §C11.5 / commit `95f87e6`); `0.9.0-hardening` (M14.5-shakedown candidate from 2026-05-10).
...
| `GS9999999998` | `0.10.0-at-timeout` (in-flash); `0.11.0-wipe-cmd` (built, awaiting flash on Jace's return — see coord §C21) | Working | iBasis trial (Nordic-shipped, 10 MB lifetime — fresh) | Provisioned 2026-05-16 per new-dev-unit-bringup playbook. **The §C11.5 patch-validation target.** Boot 6+, faults 0/0/0, hourly heartbeat publishing all extras. Bench-validated cleanly post-patch (zero AT timeouts on healthy cellular). Cloud state as of 2026-05-18T02:09:55Z (post-§C21 synthetic E2E): `ready_to_provision`, no outstanding cmds, Shadow desired empty. |
...
- ~~AA-battery-recycle: charger-gated reset (DL6) is invalid on AA hardware.~~ **DESIGN + CODE DONE 2026-05-17** (coord §C20 announcement + §C21 implementation). Software-orchestrated wipe-ack: `POST /end-assignment` → cloud publishes `wipe` cmd → firmware wipes + acks via Shadow `reported.wipe_complete` + heartbeat `last_cmd_id` echo → cloud auto-recycles `discontinued → ready_to_provision` with `battery_pct ≥ 0.10` sanity floor. Firmware 0.11.0-wipe-cmd built + committed (`5d73684`); flash to `GS9999999998` is the next firmware-side action. Cloud B-series deployed + 5-stage synthetic E2E validated (T-W1/W2/W3/W5a/W5b/W6 all pass). Full design memo: `gosteady-portal/docs/specs/2026-05-17-aa-battery-recycle.md`.
- Phase 1C offline detector Lambda (`lastSeen > 2 hr`) — surfaced as gap by the conference silent-failure (cap dark for 3 days 21 hrs, no alarm fired). Coord §C11.7 has the slim-scope sketch (EventBridge scheduled rule + Lambda scans Device Registry for `active_monitoring` devices with `lastSeen > 2 h`). Currently the highest-priority cloud increment per coord §C21.8.
...
- **Coord doc (append-only joint log):** `~/Library/Mobile Documents/com~apple~CloudDocs/Documents/GoSteady/gosteady-portal/docs/firmware-coordination/2026-04-17-cloud-contracts.md`
  ...
  - **Latest entry: §C21 (2026-05-17)** — AA-battery-recycle (DL6 rewrite) implementation shipped + synthetic E2E validated; firmware 0.11.0-wipe-cmd built but not yet flashed
...
- **Cloud-side state (2026-05-17):** Phases 0A-rev / 0B-rev / 1A-rev / 1B-rev / 1.5 / 1.6 / 1.7 deployed; Phase 2A split into 6 subsets — 2A-0 foundation + 2A-DL device-lifecycle deployed; 2A-RD / 2A-AA / 2A-UM / 2A-INT planned. AA-battery-recycle cloud B-series deployed (audit_catalog + heartbeat-processor + device-shadow-handler + device-api + observability L17 alarm) — coord §C21. 1C (offline detector), 2B (portal integration), 2C (notifications), 3A/B (hosting + CI/CD) all 🔲 planned. Next firmware-side action: flash 0.11.0-wipe-cmd to `GS9999999998` + bench-validate the wipe roundtrip (closes coord §C22).

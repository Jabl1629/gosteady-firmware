...
| `GS9999999981` | `rol-0.1.0-bench` (**ROLLATOR product** — first rollator bench unit; flashed 2026-07-01, coord §C49; fresh Thingy:91 X, boot 1) | Working (external J-Link) | iBasis eSIM — **UNACTIVATED: EMM cause 8 rejection on every attach (9.5 h overnight, multiple cells/TACs). SIM itself reads fine (verified 2026-07-02 via at_client): ICCID `89444611503503713990`, IMSI `204043547240713`. Claim at nrfcloud.com (+ PUK from box label), power-cycle.** | Cert sec_tag 201 verified; cloud-side fully prepped: Thing under `GoSteadyRollatorPlatform-dev`, registry `deviceType=rollator_platform` / `hardwareVariant=thingy91x_bench`, **provisioned** to `pat_dt1_rollator_bench_1782968418` (client_rd_test) with activate cmd `act_7bcdb1ca…` queued (24 h window from 2026-07-02T05:00Z — re-provision if claimed later). 9.5 h soak: faults 0/0/0. A 15 s desk session (`e7f1be80…`, rollator vocab, auto-stopped on stillness) is ON-DEVICE awaiting uart1 pull — **pull before any reboot** (boot orphan sweep). USB CDC disconnected 2026-07-02 08:23 (board on battery). Resume runbook: portal spec `phase-dt1-rollator-bench-bringup.md` §Deployment. |

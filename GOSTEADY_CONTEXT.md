...
Firmware version: **`0.17.0-time-wakewindow`** (device time reliability, 2026-06-18; coord §C47 / spec `gosteady-portal/docs/specs/2026-06-18-device-time-reliability.md`). Adopts the NCS **`date_time`** lib (NITZ → NTP → app-set) in place of the bespoke `AT+CCLK?` read, with a `[2024,2050]` plausibility gate (`src/gs_time.h`); deletes the §C11.5 bounded-timeout AT wrapper (the time path no longer touches the modem — `date_time_now()` is a cached read); adds optional activity/heartbeat fields (`clock_synced`, session/publish uptimes, `boot_count`, `time_source`) so the cloud can reconstruct timestamps from receive-time when the clock is unsynced; heartbeat stays alive when unsynced (omits `ts`). **Bench-validated end-to-end on `GS0000000001` 2026-06-18** — NITZ path (`src=nitz`), **NTP fallback proven (`DATE_TIME_MODEM=n` test → `src=ntp`, correct date: outbound UDP/123 works on the iBasis APN, spec Q2=YES)**, and a live walk uplink with all new fields + correct 2026 dating. **Cloud side also landed + deployed the same session** (`GoSteady-Dev-Processing`): `_shared/device_time.py` cloud-ingest reconstruction + heartbeat-`ts` guard (13 unit tests) + contract-doc lockstep. Only remaining: cleanup of the ~60 legacy `2080-*` rows. Prior: **`0.16.1-gait-wakewindow`** (gait speed + decoupled step counter, 2026-06-18; coord §C46 / spec `gosteady-portal/docs/specs/2026-06-07-gait-speed.md`). Prior pilot lineage: **`0.13.1-pilot`** (battery pilot, 2026-05-31; see coord §C38). Two-step battery-life work for the **1350 mAh AA-class pilot cell / 30-day target**: `0.12.0-psm` requests LTE-M **PSM** at attach (root cause was `CONFIG_LTE_PSM_REQ` never set → modem idled in registered I-DRX at ~mA 24/7 → ~20-day life; **fixed + validated on `GS9999999998`**, carrier granted tau=3 h / active=2 s), and `0.13.x-pilot` adds the `prj_pilot.conf` overlay (snippets OFF + `CONFIG_GOSTEADY_LOW_POWER`; **dark at rest but green session LED on during recording** via `CONFIG_GOSTEADY_SESSION_LED`, added 0.13.1 — the idle purple blink stays off). Bench model projects ~90–210 days optimized; per-connection TLS energy is the remaining swing, to be pinned by an on-battery SoC-slope discharge run. Deferred (capture hot-path risk): idle-sampler gating + gyro-disable. Prior shipping version **`0.11.0-wipe-cmd`** (as of 2026-05-17; commit `5d73684`; flashed to `GS9999999998` 2026-05-17 21:55 UTC; **bench-validated end-to-end 2026-05-18** — full wipe-ack auto-recycle roundtrip per coord §C22, including memo W-R2 closure via `tools/pull_sessions.py --list-only` returning 0 files post-wipe). Three structural findings surfaced at bench (coord §C22.3) — most important is AWS IoT MQTT 3.1.1 persistent_session 1h timer expires before firmware's 1h heartbeat, making downlink cmds operationally unreliable without intervention; §C23 design memo for cloud-side connection-coordinator Lambda is the production fix. Previous versions: `0.10.0-at-timeout` (bounded-timeout AT wrapper in `cellular.c`, §C11.5 / commit `95f87e6`); `0.9.0-hardening` (M14.5-shakedown candidate from 2026-05-10).
**PRODUCT SPLIT (DT-1, 2026-07-01, coord §C49):** this app now builds TWO
products via the `GOSTEADY_PRODUCT` Kconfig choice (default `WALKER_CAP` —
all pre-DT-1 overlays/builds unaffected). `GOSTEADY_PRODUCT_ROLLATOR`
(`prj_rollator_cloud.conf`, first unit `GS9999999981`) = rollator accessory
platform: version line `rol-0.1.0-*`, heartbeat `device_type:
"rollator_platform"` (cloud cross-checks vs Device Registry + alarms on
mismatch — both products now self-report), activity uplink = `active_min` +
Core envelope ONLY (walker stride metrics compile-gated out until the DT-2
wheeled-motion algo arc; the gate lives in `cloud.c
build_activity_payload`). Capture vocab gained append-only rollator entries
(`rollator_4wheel`/`frame_mount`/`accessory_platform`/`brake_stop`/
`brake_drag`/`park_brake_seated`/`heavy_lean` — session.h + control.c +
read_session.py + workbooks in lockstep). Rollator capture assets:
`tools/capture_rollator.html` (39-run matrix, isolated localStorage) +
`data collection and protocols/GoSteady_Rollator_Capture_Protocol_v1.md` +
`GoSteady_Rollator_Annotations_v1.xlsx`. Portal-side scoping/spec:
`gosteady-portal/docs/specs/2026-07-01-device-types.md` +
...
> **Canonical reference: [`docs/build-configurations.md`](docs/build-configurations.md)** — the full overlay×symbol matrix, per-device image map, and version-string ambiguity notes. The table below is a quick index; that doc is the source of truth.
...
| `build_rollator_bench/` | none + `-DCONFIG_GOSTEADY_PRODUCT_ROLLATOR=y -DCONFIG_DATE_TIME=y -DCONFIG_DATE_TIME_NTP=n -DCONFIG_GOSTEADY_MOTION_AUTOSTART=n` | **ROLLATOR capture-day image (no cloud):** for DT-2 data-collection days — no activity uplinks → no .dat auto-prune after PUBACK (the cloud build deletes each session seconds after publish once cellular is up!), no SIM spend. BLE/uart1 capture identical. NITZ-only clock. |
...
| `build_rollator_field/` | `prj_rollator_field.conf` (new 2026-07-06) | **ROLLATOR deployment baseline:** mirrors `prj_field.conf` (dark deployment posture) for the rollator + snippets OFF. Authored, not yet bench-built. |
| `build_rollator_pilot/` | `prj_rollator_pilot.conf` (new 2026-07-06) | **ROLLATOR deployment target:** field + `LOW_POWER` + `SESSION_LED` + `PREACT_LOWPOWER` — carries the walker's battery/shipping-mode bundle forward. Version `rol-0.1.0-ww`. `PREACT` wake-on-motion thresholds are walker-tuned → DT-3 bench validation (spec A3). Authored, not yet bench-built. |
| `build_rollator_dist/` | as `build_rollator_bench` + `gs_rollator_distance` module | **ROLLATOR distance-capture image** (coord §C52; on `GS9999999981` through 2026-07-06, since superseded by the `rol-0.1.0-ww` pilot flash — kept as the DT-2 capture restore point). Capture-day posture: no cloud, `MOTION_AUTOSTART=n` (so motion does **not** autowake / light the green LED — by design), NITZ-only clock. Reports `rol-0.1.0-bench` (same string as the cloud/field builds — see build-configurations.md §5). |
...
| `GS0000000001` | `0.17.0-time-wakewindow` (flashed 2026-06-18, coord §C47 — device time reliability; was `0.16.1-gait-wakewindow` flashed 2026-06-18, coord §C46 — gait + step-counter; was `0.15.3-wakewindow` flashed 2026-06-04, coord §C45 — **wipe→shake reactivation hang fix**; was `0.15.1-wakewindow` observed live 2026-06-03. client_id baked + cert sec_tag 201. Superseded the `0.14.0-shipmode` of coord §C39/§C40 via the 0.15.0/0.15.1 wake-window commits) | Working (external J-Link) | iBasis eSIM (fresh) | **First shipping unit. As of 2026-06-03 (coord §C41): facility-activated end-to-end through the live portal.** Provisioned + activated via the facility build (`dev.portal.gosteady.co`) under `client_dev_pilot`/`fac_dev_pilot_a` (cmd `act_353d21c0…`, delivered on a shake via the 0.15.0 wake-window). **D2C QR-claim staging was overwritten by this** — the `walkerId 37b2e250-…` attribute persists but the unit is now facility-owned (not a claimable D2C cap); re-stage for D2C per coord §C41.3 if that test is wanted back. **As of 2026-06-04 (coord §C42) it ran the full device lifecycle:** Pilot CapTest (42-step walk) → **End Monitoring** → wipe + auto-recycle to `ready_to_provision` (on a shake; `reported.wipe_complete` matched) → re-provisioned + re-activated to a NEW resident (Rosa Delgado, `pat_8ce10709…`, 16-step walk). Currently **`active_monitoring` for Rosa**, battery ~94%. **2026-06-04 (coord §C45):** flashed `0.15.3-wakewindow` + console-validated the wipe→shake reactivation fix — re-ran End-Monitoring → wipe → Resume → **shake reactivated with NO reboot** (`preact: ACTIVATED during wake window → green confirm`; boot_count steady, 3.1 h continuous uptime). The §C44.5 "reboot needed post-wipe" trap is fixed; the §C44.5 PSM/rate-limit hypothesis was wrong (real cause: heartbeat thread parked in the activated `k_sleep` across the wipe). **2026-06-18 (coord §C46/§C47):** flashed `0.16.0`→`0.16.1-gait` (decoupled merge-0.8 step counter + `gait_speed_fts` + long-session guard relax); step count + gait validated end-to-end (active_monitoring for Rosa; gait 0.55–1.23 ft/s rendering in the portal). Investigating "missing" Jun 15–16 portal data revealed the device clock read **2080** on Jun 14–16 (roaming iBasis SIM gave no NITZ → modem 1980 default; parser accepts it; ~60 walks mis-dated but **not lost** — confirmed online via heartbeats + real `ingestedAt`). Time-reliability fix **firmware-side implemented + bench-validated 2026-06-18 (coord §C47, `0.17.0-time`):** SNTP via NCS `date_time` lib (replaces `AT+CCLK?`; deletes the §C11.5 AT wrapper) + `[2024,2050]` plausibility gate (`src/gs_time.h`) + new activity/heartbeat time fields. Validated on this unit: NITZ path `src=nitz`; **NTP fallback proven** (`DATE_TIME_MODEM=n` test → `src=ntp`, correct date — **UDP/123 works on the iBasis APN, spec Q2=YES**, so firmware alone fixes the no-NITZ case); live walk uplink carried `clock_synced/uptimes/boot_count/time_source` + correct 2026 dating; RAM 60%. **Cloud side also landed + deployed same session:** activity-processor cloud-ingest reconstruction + heartbeat-`ts` guard (`_shared/device_time.py`, 13 unit tests, `GoSteady-Dev-Processing` deployed + smoke-verified). Live Shadow shows `clock_synced=true`/`time_source=nitz`/correct `ts`. Only remaining: cleanup of the ~60 existing `2080-*` rows. |
| `GS9999999981` | `rol-0.1.0-ww` (**ROLLATOR pilot/deployment posture** — `prj_rollator_pilot.conf`: FIELD_MODE + PSM + LOW_POWER + SESSION_LED + PREACT_LOWPOWER + `gs_rollator_distance`; flashed 2026-07-06, app RAM 63.05%, VERIFY_READ ok. Prior: `rol-0.1.0-bench` capture image `build_rollator_dist` through 2026-07-06 §C52 — its on-device session pulled to `raw_sessions/2026-07-06-pre-pilot-flash/` before the reflash. Boot 24, faults 0/0/0) | Working (external J-Link 802006700, SW2=nRF91) | **Onomondo SIM — WORKING** (user-installed 2026-07; supersedes the dead iBasis eSIM). Boot 2026-07-06 uart0: `registered_roaming` LTE-M, **PSM granted tau=3 h / active=2 s**, NITZ time OK (`src=nitz`, correct 2026 date), rsrp −92 dBm, battery ~100% (4199 mV). (The prior iBasis eSIM's EMM-cause-8 note is obsolete.) | **Activated 2026-07-02T16:16:40Z** (cmd `act_7bcdb1ca…`; provisioned to `pat_dt1_rollator_bench_1782968418` / client_rd_test). `activation.bin` persisted across the pilot reflash (ext-flash preserved) → boots activated → hourly heartbeat + motion-autostart sessions live; connecting to AWS IoT. Cert sec_tag 201; Thing `GoSteadyRollatorPlatform-dev`, registry `deviceType=rollator_platform`/`hardwareVariant=thingy91x_bench`. **Now the end-to-end rollator test unit** — distance/gait activity uplink pending cloud-side field promotion (§C52). |
| `GS0002000001` | `rol-0.1.0-ww` (**FIRST PROD ROLLATOR — deployment build** `prj_rollator_pilot.conf`; flashed 2026-07-12 for the DT-3 dry-run. Was `prj_rollator_cloud`/`rol-0.1.0-bench` for the first prod activation. Carries the **cmd_id-ack `0dc3523`** + **immediate-heartbeat `5324889`** fixes) | Working (external J-Link 802006700, SW2=nRF91; **flash via `nrfutil` — standalone `nrfjprog` is JLink-broken here, -256→-102**) | **Onomondo (working)** | **First real PROD activation (coord §C58).** Fresh board provisioned via `tools/bringup-prod-unit.sh` (prod cert sec_tag 201, Thing `GoSteadyRollatorPlatform-prod`, prod pool `us-east-1_WIWD2WXaq`). Activated E2E through the live prod D2C app (`app.gosteady.co`, SMS-OTP claim) → real rollator session (active_min/dist, no steps). Surfaced+fixed the cmd_id-ack truncation. **DT-3 (2026-07-12):** dark→shake→**BLUE wake window** (walker-tuned threshold fires on rollator)→no-green (gate holds)→re-provision→shake→**green confirm→`active_monitoring`**. Currently `active_monitoring` on the pilot build, test household `dtc_bc7f6e2cd956`. |

**Bench fleet snippet:** keep snippets enabled on `GS9999999998` for full pipeline validation; if iBasis trial nears exhaustion (snippets dominate ~300× heartbeats per the playbook sizing table), rebuild with `-DCONFIG_GOSTEADY_SNIPPET_ENABLE=n` for longevity. The rollator unit (`GS9999999981`) ships snippets OFF (RAM + SIM budget, spec D9).
...
### ⚠ OPEN BUGS — rollator capture unit (2026-07-04, coord §C51; Path B PARKED per §C52)

> **§C52 correction:** the §C51.3 "15 runs lost" was WRONG — 16 sessions were
> recovered (LIST the device before declaring capture lost; console-log gaps ≠ no
> recording; Bug 1 is RX-biased, not dead air — commands arrive + record, only
> confirmations are missing). The two bugs below are **de-prioritized (Path B
> parked)** — the workarounds suffice; the data was never actually lost.

- **Bridge uart1 enable-on-USB-only:** the nRF5340 bridge fork only enables
  uart1 when a USB CDC host opens the port; BLE-only cold boot = no *confirmations*
  back to the page (looks like dead air; but commands DO arrive + record). Enable
  persists once set → WORKAROUND: after every power-on, plug USB + open
  uart1 once (`tools/control.py status`), then BLE works all boot. FIX (parked):
  ...
  flash SW2→nRF53 via `nrfutil device` (see flash gotcha below).
...
- **`west flash` with nrfutil runner hits `UICR data is not erasable`** on nearly every flash.
- **⚠ The standalone `nrfjprog` CLI is BROKEN on this machine (2026-07-06, §C52):** it's bound to the stale system SEGGER JLink **V9.34a** and can't open the probe (`JLinkARM -256` at DLL-open; NOT a probe/cable fault — re-plug + process-kill don't help, and `JLinkExe` opens the same probe fine). **Flash via `~/.nrfutil/bin/nrfutil device`** (device plugin bundles a current JLink), or the nRF Connect GUI / VS Code. App flash used: `~/.nrfutil/bin/nrfutil device program --firmware <merged.hex> --serial-number 802006700 --core Application --options chip_erase_mode=ERASE_ALL,ext_mem_erase_mode=ERASE_NONE,verify=VERIFY_READ,reset=RESET_SYSTEM` (confirm SW2=nRF91 first via `device device-info`). `~/.nrfutil/bin/nrfutil` is the modern core; `~/.local/bin/nrfutil` is old pc-nrfutil (no `device`).
...
- **Coord doc (append-only joint log):** `~/Documents/gosteady-portal/docs/firmware-coordination/2026-04-17-cloud-contracts.md` _(moved out of iCloud 2026-05-23 to fix tsc/npm slowness; iCloud copy at the old path may persist for a short confidence-window before deletion)_
  ...
  - **Latest entry: §C67 (2026-09-23)** — **Family Assistance FA-0 hardware gate passed on an unmodified Thingy:91 X (GS0002000003, now the bench unit).** Qwiic Buzzer answers in 82 ms; cadence and all five hold-to-cancel cases pass, including the new **deferred-send rule** (never transmit while the button is down: a hold in progress at T20 defers the send until release or cancels at 3 s — spec D21). Hardware truth: P1 is behind the TXS0102; the buzzer's 2.2 k pull-ups need `nordic,drive-mode = NRF_DRIVE_H0D1` on the i2c2 pins (in `assist_buzzer_i2c2.overlay`) or the sensors drop off the bus while the rail is on. Bench hooks on uart1 (`/dev/cu.usbmodem1105` @ 1 Mbaud): `PRESS`, `HOLD <ms>`, `PING`. Still owed for FA-0: buzzer current + rail sag, loudness through the enclosure, GNSS TTFF. **(prior) §C66 (2026-09-22)** — **Field battery analysis GS0002000005: ~29 days full → dead at ~1.8 mA** (portal `docs/specs/2026-09-22-field-battery-analysis-GS0002000005.md`). Fixed cost dominates: ~70 connects/day (24 heartbeats + ~45 per-session uploads, `AUTO_STOP_STATIONARY_S=15`) at cell-edge signal (RSRP median −107 dBm); offline episodes draw ~8 mA and end in the §C62 LittleFS fatal; no low-battery cut-off → brownout boot loop (`boot_count` 8 → 1310 while dead). Unit dead 34 % of the time since activation. **Fleet cross-check (§C66.5): idle floor is 0.6 mA** (unactivated units lose 1.1 pts/day on a daily heartbeat) — field builds still carry `CONFIG_SERIAL/UART_CONSOLE/LOG_BACKEND_UART=y` + `UART_INTERRUPT_DRIVEN=y`, no `PM_DEVICE` (leading suspect; bench-measure first). Levers in order: console/UART off in field builds (−12 mAh/d), batch uploads into the heartbeat (= §C62 derived-record queue, −13), RAI + TLS resumption (−3), LED duty-cycle, 2-h heartbeat, auto-stop hysteresis, low-battery ship-mode + offline back-off. Optimized ≈ 7–15 mAh/day; a one-year pack is ~5.5–11 Ah nominal (+8 Ah if the floor is hardware). **(prior) §C65 (2026-09-22)** — **PRD V2.3 Draft created** (`GoSteady PRD V2.3 Draft - Rollator and Family Assistance.docx`, iCloud `Documents/GoSteady/`): spec §15 applied — buzzer beep countdown + press-and-hold 3 s cancel are now PRD requirements (AST-HW-03/HW-04/FW-02/FW-04/FW-06, §6 exclusion, §7 gates); V2.2 untouched; no code change. **(prior) §C64 (2026-09-18, night)** — **Family Assistance pivot: DFR0534 speaker RETIRED → SparkFun Qwiic Buzzer (BOB-24474, I²C 0x34) on the P1 Qwiic connector; 20 s beep countdown, press-and-HOLD 3 s cancels.** Feedback abstraction `src/feedback.h` with backends `feedback_buzzer.c` (product) / `feedback_speaker.c` (archived); `src/assist.c` rewritten for hold-to-cancel + cadence phases (10 s double beep, 17 s rapid). Overlays: `boards/assist_buzzer_i2c2.overlay` (unmodified boards — no cutting needed) / `boards/assist_buzzer_bitbang.overlay` (the cut bench unit, Zephyr `gpio-i2c` on P0.18/P0.19). `prj_assist_bench.conf` = buzzer bench (RAM 52.9 %); `prj_assist_speaker_bench.conf` = archived speaker bench. Buzzer on order — not yet exercised. Spec v0.4 §15 carries the PRD V2.3 amendment text. **(prior) §C63 (2026-09-18)** — **Family Assistance Alert umbrella spec drafted** (`gosteady-portal/docs/specs/family-assistance-alert.md`, PRD V2.2 §5): assistance button (SW0/P0.26) + DFR0534 speaker on P1 + `gs/{serial}/assist` uplink + `assist_ack`/`assist_arm` cmds + GNSS-after-ack, cloud fan-out to Care Circle via Retell voice + Twilio SMS. **Bench harness built (same day, §C63.5):** `prj_assist_bench.conf` + `boards/assist_audio_uart1.overlay` + `src/assist.c` + `src/audio_dfr0534.c` — SW0 → DFR0534 prompts with a stub cloud ack; **BUTTON → SPEAKER PROVEN on the bench 2026-09-18 (§C63.6):** SB8/SB9 cut, `build_assist_bench` flashed, full 20 s sequence ran with the module's factory clips; module answers 570 ms after power-up. Canonical pins: **our TX = P0.18 (P1 pin 3, EXP_BOARD_PIN2, blue → module "T"), our RX = P0.19 (P1 pin 4, EXP_BOARD_PIN1, yellow → module "R")** — DFRobot labels T/R from the host side. **Never flash an audio image onto an uncut board** (uart1 TX would fight the I²C SDA/SCL). Real prompt WAVs still need a micro-USB *data* cable to load (`tools/load_dfr0534_prompts.sh`). Cloud transport not wired (FA-2). Firmware-gating facts: **no spare UARTE** (uart2↔i2c2, uart3↔spi3 share slots → re-pin uart1 or use an I²C-UART bridge), SB8/SB9 topology now resolved (see above), audio must be power-gated via `exp_board_enable` (P0.03), and `mcuboot-button0` serial-recovery entrance must be disabled in deployment images. **(prior) §C62 (2026-07-31)** — session-storage exhaustion → LittleFS SAU fault on GS0002000002 (spec `docs/specs/2026-07-31-session-storage-exhaustion.md`, unfixed, HIGH). **(prior) §C54 (2026-07-08)** — **DT-4 D2C launch work: all 4 workstreams implemented + backend/facility-frontend deployed to dev; first browser test surfaced 3 gaps.** Rollator D2C dashboard renders active-min/distance/gait (no steps); behavioral rules re-keyed to activeMinutes; SMS-OTP proven live; `status_patientId` D2C bug fixed. Gaps for next session: the live D2C app (`main_d2c.dart`) is **not hosted** (ran locally for the test); the dashboard hit **`401`** (D2C Cognito JWT not accepted by the API authorizer — fix first); email+SMS is **double verification** (UX). Code done + pushed (portal `4289809`/`6dc62d2`/`318b0c6`). **(prior) §C53 (2026-07-06)** — **Rollator pilot flashed → end-to-end live + cloud distance/gait promotion + a ProcessingLambda bundling fix.** `rol-0.1.0-ww` on GS9999999981 (Onomondo SIM, activated, PSM/NITZ OK); device→cloud→named `distanceFt`/`gaitSpeedFts` columns (steps dropped for rollator — D10 amended), deployed + synthetic-validated; deployment overlays (`prj_rollator_pilot`/`field`) + canonical `build-configurations.md` added; found+fixed a pre-existing asset-hash bug where `_shared`-only Lambda changes silently didn't deploy. **DT-4 (D2C dashboard rendering) unblocked — Twilio approved 2026-07-06.** **(prior) §C52 (2026-07-06)** — **Rollator distance estimator: ported to firmware + flashed + on-device verified.** Continuous flat-norm vibration odometer (`src/algo/gs_rollator_distance.c`, compile-gated `PRODUCT_ROLLATOR`, no FFT/no classifier; golden ref `algo/rollator_distance_ref.py`, host-parity 44/44 @ 0.02%). **NEW cloud-contract fields:** rollator activity now emits `distance_ft` + `gait_speed_fts` (confidence-gated) — **cloud validators/activity-processor must accept them for `rollator_platform`**. Honest arc: initial "17%≈oracle" was a constant-speed artifact (adversarial-review-caught), re-validated on variable-speed 3-surface data (~26–31% MAPE, walker parity). §C51.3 "15 runs lost" **RETRACTED** (16 recovered; Path B parked). Flash via `nrfutil device` (standalone nrfjprog CLI broken — stale JLink V9.34a). **(prior) §C47 (2026-06-18)** — **Device time-reliability — firmware landed + bench-validated** (`0.17.0-time`). Fixes the Jun 14–16 **2080-timestamp** incident on `GS0000000001` (roaming SIM, no NITZ → 1980 modem default → ~60 walks mis-dated 2080; data NOT lost). Firmware adopts the NCS `date_time` lib (NITZ → NTP → app-set), deletes the §C11.5 `AT+CCLK?` wrapper (time path no longer touches the modem), adds a `[2024,2050]` plausibility gate (`src/gs_time.h`) + optional activity/heartbeat time fields (`clock_synced`, session/publish uptimes, `boot_count`, `time_source`), and keeps the heartbeat alive when unsynced. **Bench-proven on `GS0000000001`:** NITZ `src=nitz`; **NTP fallback `src=ntp` with `DATE_TIME_MODEM=n` → UDP/123 works on the iBasis APN (spec Q2=YES), so firmware alone fixes the no-NITZ case**; live walk uplink carried the new fields + correct 2026 dating; RAM 60%. **Cloud side also landed + deployed same session:** activity-processor cloud-ingest reconstruction (`ingestedAt − age`) for `clock_synced=false` + heartbeat-processor `lastSeen`-from-`ingestedAt` guard (`_shared/device_time.py`, 13 unit tests; `GoSteady-Dev-Processing` deployed + smoke-verified, validation-contract change live), contract-doc + ARCHITECTURE §6/§7 lockstep done, spec status → implemented (coord §C47.4). Only remaining: cleanup of the ~60 existing `2080-*` rows. Spec `gosteady-portal/docs/specs/2026-06-18-device-time-reliability.md`. **§C46 (2026-06-07)** — **Gait speed, firmware→cloud→portal** (`0.16.0-gait`): decoupled merge-0.8 step counter (raw impulse over-count 47%→16% step MAPE, slow-walk 78%→7%, zero distance cost) + new optional `gait_speed_fts` activity field (`distance_ft` ÷ peak-train walking time) + long-session guard (relaxed to peak-cap only in `0.16.1`); shipped + live on `GS0000000001`. **(prior) §C45 (2026-06-04)** — **Firmware fix: wipe→shake reactivation hang (`0.15.3-wakewindow`).** A console-attached bench repro overturned §C44.5's guess (PSM/rate-limited motion-connect — never validated): the wake window opened but **never connected**, even on USB with a warm modem. Real cause: the heartbeat thread's activated branch blind-`k_sleep(HEARTBEAT_INTERVAL)` (1 h) doesn't re-check activation, so a wipe (activated→pre-activation) mid-sleep leaves it parked in the activated branch NOT serving wake windows → a shake's blue-pulse window has no connection → queued `activate` never collected (only a reboot recovered it). Fix: interruptible activated sleep (`k_sem_take(&heartbeat_wake_sem, …)`) + `gosteady_activation_clear()` gives the sem via `gosteady_cloud_notify_deactivated()` (`cloud.c`/`activation.c`/`cloud.h`). Console-validated on `GS0000000001`: End-Monitoring → wipe → Resume → **shake reactivated with no reboot** (`preact: ACTIVATED during wake window → green confirm`), wake-window motion timeout intact (un-queued first window ended at the 600 s hard cap → ship sleep). No cloud contract change. **(prior) §C43 (2026-06-04)** — **Fuel-gauge fix: voltage-based (OCV) SoC.** A `GS9999999998` discharge run exposed the `nrf_fuel_gauge` coulomb estimate diverging to 0% at sub-mA idle (read 0% / `battery_critical` at 4.086 V ≈ 86% actual; 12% at a full 4.213 V) — battery fine, gauge broken (FMEA 3.1). Replaced with a voltage→OCV lookup (`battery.c`, unconditional; `0.12.1-psm` / `0.13.2-pilot` / `0.15.2-wakewindow`); verified 4.213 V→100%, Shadow `battery_pct=1.0`. Earlier GS98 `battery_critical` alarms were false. No cloud change. **(prior) §C42 (2026-06-04)** — **Device-lifecycle UX consolidation ("End Monitoring") + discharge-cascade wipe fix; full single-cap lifecycle validated.** Dropped "Discontinue Device", renamed "Discharge Resident" → **"End Monitoring"** (no reason — just confirm + optional notes; backend discharge `reason` now optional). Fixed the `discharge-cascade` Lambda that never queued the wipe (caps released via discharge got stuck in `discontinued`, never recycling to the pool) — now mirrors the end-assignment AA-recycle write; the §C24 coordinator delivers the wipe on next connect. **Validated the ENTIRE device lifecycle on one physical cap** (`GS0000000001`, `0.15.1-wakewindow`, unmodified): Pilot CapTest provision→activate→walk 42 steps → End Monitoring → wipe + recycle (on a shake) → Rosa Delgado re-provision→re-activate→walk 16 steps. **Zero firmware change.** **(prior) §C41 (2026-06-03)** — **First physical-device activation through the FACILITY portal, end-to-end on the live site** (`dev.portal.gosteady.co`): caregiver sign-in → Add Resident + provision `GS0000000001` → activate cmd → Jace shook the cap (0.15.0 wake-window) → `active_monitoring` → walk (42 steps / 50.09 ft / outdoor) rendered on the caregiver's screen. Phase 2B umbrella exit, hardware in the loop. **3 live-mode portal bugs fixed + deployed** (Flutter Unit-dropdown value-equality; `patient-mgmt` `status_patientId` missing on create → residents invisible to roster; + not updated on discharge). **D2C coordination follow-ups logged** (`deploy-portal.sh --delete` clobbers `/d2c/` in the shared bucket; `d2c-claim` `status_patientId` uses `active#` vs canonical `active_`; `GS0000000001` D2C↔facility device contention). **Zero firmware change** — `0.15.1-wakewindow` ran unmodified. **(prior) §C40 (2026-05-31)** — Pre-activation **shipping mode** (`0.14.0-shipmode`): gated sampler (100 Hz sampler blocks when idle instead of spinning — **soak-validated**, 16 start/stop cycles, 0 faults) + blue-on-motion pre-activation indicator + 24 h safety-net heartbeat. Cuts pre-activation storage draw ~340-410 → ~48 mAh/month (months of shelf life, no pull-tab). Flashed to `GS0000000001`; D2C staging intact. Spec: `gosteady-firmware/docs/specs/preactivation-lowpower-mode.md`. **(prior) §C39 (2026-05-31)** — `GS0000000001` flashed with `0.13.1-pilot` (client_id baked + cert sec_tag 201) and staged for the first **D2C QR-claim test**: walkerId/QR assigned, public lookup `unclaimed`, device heartbeating in pre-activation (`ready_to_provision`, owner NULL, `activated_at` None → real activate flow). Blocked only on Twilio OTP (operator) + Jace's phone-side claim. **(prior) §C38 (2026-05-31)** — Firmware battery pilot: LTE-M PSM (`0.12.0-psm`) + low-power overlay (`0.13.1-pilot`, `prj_pilot.conf`; idle dark + green session LED on). Root cause of the 30-day-budget overrun was `CONFIG_LTE_PSM_REQ` never set (modem idled at ~mA 24/7). PSM validated on `GS9999999998` (carrier granted tau=3 h/active=2 s). No cloud contract change; downlink (activate/wipe) unaffected (still delivered at next connect via the §C24 coordinator). Next: on-battery SoC-slope discharge run to measure real life. **(prior) §C37 (2026-05-31)** — D2C Phase 1 backend (walker-user claim + activation) deployed to dev. New consumer/household product on a 5-phase plan (walker-claim → SMS → deactivation/reset → new-user-reuse → caregivers); specs `d2c.md` + `d2c-phase1-walker-activation.md`. New `GoSteady-Dev-D2C-Auth` stack: separate Cognito pool (`us-east-1_bhvtxuHwD`), SMS-OTP custom-auth (via **Twilio** — dev AWS account has no SNS SMS origination), D2C pre-token. `d2c-claim` Lambda: `POST /api/v1/claim` (2nd JWT authorizer) bootstraps household+Patient+RoleAssignment then provisions the device via the existing 2A-DL activate path; public `GET /api/v1/public/walkers/{walkerId}` drives the QR `/setup` landing; new `by-walker-id` GSI (opaque QR id → serial; GS serial never exposed). Synthetic e2e all-green (3 live-AWS bugs found+fixed: empty-set, error_response arity, status_patientId GSI key). **Zero firmware-facing impact** — reuses shipped activation/provision/Shadow-desired contracts verbatim. Blocked on operator: Twilio compliance review (~2 biz days) + populate `gosteady/dev/twilio` secret before OTP sends; real-device exit test pending (decouplable from SMS via admin-minted token). Prior: §C36 (2026-05-27) FAC-W follow-up bundle.
...
- **Absolute time from the NCS `date_time` lib** (`cellular.c`, 0.17.0-time, coord §C47) — `date_time_now()` / `date_time_uptime_to_unix_time_ms()` are cached RAM reads (NITZ via the %XTIME push → NTP/SNTP → app-set), so the time path no longer issues any modem AT command. A `[2024,2050]` plausibility gate (`src/gs_time.h`) makes implausible (1980/2080) values return `-EAGAIN`. This **replaced** the bounded-timeout AT wrapper below (now deleted) and structurally fixes the no-NITZ 2080 mis-dating; the cloud reconstructs from receive-time when `clock_synced=false`.
- **~~Bounded-timeout AT wrapper (`at_cmd_with_timeout`, 2 s) for `session_start` CCLK calls~~ (REMOVED in 0.17.0-time — superseded by the `date_time` cached read above; the time path no longer touches the modem so the wrapper + its `_bare` self-deadlock workaround are gone)** (`cellular.c`, 0.10.0-at-timeout) — synchronous `nrf_modem_at_*` calls can block past the 60 s WDT envelope under sustained modem contention (e.g. SIM-exhausted PDN-reject loop, as observed 2026-05-11/12 conference). Wrapper dispatches to a worker that runs INSIDE the reporter thread (RAM consolidation; dedicated thread overflowed by ~2 KB). Caller gets `-ETIMEDOUT` after 2 s instead of an indefinite block; maps to `-EAGAIN` so existing FMEA 1.1 retro-stamp handles `session_start` proceeding without cellular UTC. **Reporter-thread AT calls MUST bypass the wrapper** (use `_bare` variants like `read_network_time_iso8601_bare()`) or they self-deadlock: the wrapper gives `at_request_sem`, but the reporter is the only consumer and is blocked inside the wrapper waiting for itself. The 2 s timeout saves from a true deadlock but produces spurious `at cmd timed out (modem contention?)` warnings every cycle. See coord §C11.5 / §C12.4 for full design + the bench-discovered self-deadlock that motivated the `_bare` pattern.

...
- **NEW (high-priority): connection-coordinator Lambda** to address §C22 Finding 2 (AWS IoT MQTT 3.1.1 persistent_session 1h timer expires before firmware's 1h heartbeat, making cmds queued by broker between firmware connects unreliable). Design memo coming as §C23. Production-blocker before first prod customer — without it, any downlink cmd flow has no guaranteed delivery to a device on hourly cadence.
...
- **Coord doc (append-only joint log):** `~/Library/Mobile Documents/com~apple~CloudDocs/Documents/GoSteady/gosteady-portal/docs/firmware-coordination/2026-04-17-cloud-contracts.md`
  ...
  - **Latest entry: §C22 (2026-05-18)** — AA-battery-recycle physical-device bench validation: full wipe-ack auto-recycle roundtrip succeeded on `GS9999999998`; three structural findings filed (most important: AWS IoT 1h persistent_session timer)

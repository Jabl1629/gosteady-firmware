...
Milestones 1–6 are done. Immediate next: a **full rehearsal of the v1 30-run protocol over BLE with the cap sealed** to validate the `capture.html` → bridge → uart1 → parser → flash → post-session USB dump → ingest pipeline end-to-end before real data collection (M8). Any friction points that show up there feed back as small fixes. After that, the arc is essentially M8 → M9 (Python algorithm) → M10 (C port) etc. — a dedicated Python CLI that wraps BLE control + USB pull (originally scoped as M7) may end up being unnecessary given the operator UI already lives in `tools/capture.html` plus `tools/control.py` for bench work. 

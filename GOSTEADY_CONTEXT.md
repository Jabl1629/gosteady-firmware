...
- **Coord doc (append-only joint log):** `~/Documents/gosteady-portal/docs/firmware-coordination/2026-04-17-cloud-contracts.md` _(moved out of iCloud 2026-05-23 to fix tsc/npm slowness; iCloud copy at the old path may persist for a short confidence-window before deletion)_
  ...
  - **Latest entry: §C34 (2026-05-27)** — CORS drift incident on dev API gateway: `cdk deploy` without `--force` had silently skipped the `https://dev.portal.gosteady.co` CORS allowOrigins entry over multiple prior deploys, breaking the portal with `ApiException(NETWORK, 0)` even though the API itself was healthy. Force-redeployed `GoSteady-Dev-Api` with `--force`; CFN updated `HttpApi`; portal recovered. Lessons recorded in coord §C34.3: ApiClient swallows underlying exception type, masking CORS as generic "Connection lost"; CDK can silently reuse stale `cdk.out`; CORS post-deploy smoke check (one-liner curl) should join every API stack deploy runbook. **Zero firmware-facing impact**. Prior 2026-05-26 work: §C33 alert recurrence policy; §C32 L5 throttle; §C31 TTL caching; §C30 L6 notification swap; §C29 PollingController; §C28.5 CR-1 deploy

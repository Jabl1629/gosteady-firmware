...
- **M10.7 production-telemetry stack + M12.1c.2 production-shaped heartbeat — code-complete 2026-04-27 (4 commits, all 4 milestones, builds clean for bench/cloud/field; bench-validation deferred to next bench session).**
  ...
  - **Hardware bench-validation status:** all four milestones built clean, no flash/run cycle yet (J-Link Mini was throwing `-256` at the time; flash + uart0 stream watch is the next bench-session activity). Useful smoke checks once flashed: (a) on first boot after the M10.7.1 repartition, `boot_count` should reset to 1 because the previous LittleFS partition was untouched but the new partitions are uninitialised — actually no, `littlefs_storage` is at the same address+size so `/lfs/boot_count` survives; (b) forensics first-boot log line should be `forensics: boot=N reset=POWER_ON faults=0 wdt=0` (or `reset=PIN` if reflashed via SWD); (c) hourly heartbeat ticks should show in uart0 with all extras populated; (d) cloud Shadow `reported.fault_counters.fatal` etc. become observable.

...
- **Coord doc (append-only joint log):** `~/Documents/gosteady-portal/docs/firmware-coordination/2026-04-17-cloud-contracts.md` _(moved out of iCloud 2026-05-23 to fix tsc/npm slowness; iCloud copy at the old path may persist for a short confidence-window before deletion)_
  ...
  - **Latest entry: §C28.5 (2026-05-26)** — CR-1 heartbeat-processor lastSeen-mirror deployed + live-validated against `GS9999999998`. First post-deploy hourly heartbeat at 16:42:23Z populated `Device Registry.lastSeen` (was `null` forever on live devices); `patient-api` `currentDevice.lastSeen` now returns the real timestamp instead of the stale `firstHeartbeatAt`. Dev portal at `dev.portal.gosteady.co` now renders "Last seen: 1 min ago" on Patient Detail. **Zero firmware-facing impact** — cloud-side state-correctness fix only. Prior §C28.1–.4 covered the diagnosis + initial fix during the 2B-FAC-R real-data smoke. Prior milestones: §C27 (2026-05-24) three cloud deploys — 2A-0 amendment + 2A-UM-P + 1C-slim; §C24 (2026-05-18) connection-coordinator Lambda live; §C26 (2026-05-23) Phase 2A-AA alert actions deployed

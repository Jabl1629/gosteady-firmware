...
- **Coord doc (append-only joint log):** `~/Documents/gosteady-portal/docs/firmware-coordination/2026-04-17-cloud-contracts.md` _(moved out of iCloud 2026-05-23 to fix tsc/npm slowness; iCloud copy at the old path may persist for a short confidence-window before deletion)_
  ...
  - **Latest entry: §C35 (2026-05-27)** — 2B-FAC-W facility writes deployed + live-validated. All five caregiver actions (alert ack, add resident, edit/discharge/pause/resume, care note) wired through the FacilityRepository against 2A-AA + 2A-UM-P endpoints (all already deployed). New `CareNotePanel` + `PauseBanner` widgets on Patient Detail. Coord §C33 manual-ack-release path verified live for the first time: PATCH /alerts ack with caregiver Cognito sub → `Patient.openAlerts.<type>` slot released; subsequent threshold breach claims a fresh slot + writes new alert. ApiClient error-detail preservation closes coord §C34.3 lesson #1. **V1 caregiver UX feature-complete.** Census-tier paused-bell icon (US-31) deferred — needs `/me/patients` augmentation. **Zero firmware-facing impact** — 5 portal-only mutations on existing endpoints. Prior milestones: §C34 (same day) CORS drift recovery; §C33 (2026-05-26) alert recurrence policy; §C32 L5 throttle

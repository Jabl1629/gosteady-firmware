...
The repo is at a **session-logging bring-up target**. On boot: mount LittleFS, bump the boot counter, start a 100 Hz BMI270 sampling thread with a decoupled writer, wire SW0 as a short-press toggle for session start/stop, bring up the uart1 dump channel. LED state tells the operator which mode they're in (solid green while recording, purple blink while idle). As of **2026-04-20**, **Milestones 1–5 are complete**: toolchain, SDK, J-Link path, build+flash cycle, serial console, sensor drivers, LittleFS, session-file writer, and a host-pull path over uart1 are all verified against real hardware.
...
Milestones 1–6a are done. Next is **Milestone 6b — nRF5340 BLE bridge firmware**: a combined "USB CDC bridge + BLE peripheral" image for the nRF5340 that (a) preserves the two existing CDC-ACM endpoints (console on uart0, dump/control on uart1) and (b) adds a Nordic UART Service that transparently tunnels NUS RX bytes → nRF9151 uart1 and uart1 → NUS TX notifications. Rationale: the v1 walker cap seals the nRF9151 USB port inaccessible during runs, so session start/stop has to be wireless; phone-in-pocket + nRF Connect is the right UX for v1 operators. The nRF9151 firmware doesn't change — the M6a parser already works transport-agnostically. Risk: bridge-reflash has a history of "board stops enumerating" (documented in Known Gotchas); recovery path via SW2+nrfjprog is known. 

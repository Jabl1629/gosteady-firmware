...
- **Coord doc (append-only joint log):** `~/Documents/gosteady-portal/docs/firmware-coordination/2026-04-17-cloud-contracts.md` _(moved out of iCloud 2026-05-23 to fix tsc/npm slowness; iCloud copy at the old path may persist for a short confidence-window before deletion)_
  ...
  - **Latest entry: §C32 (2026-05-26)** — 2B-FAC-R L5 row-loader throttle landed in portal: new `lib/state/row_loader_queue.dart` (generic RowLoaderQueue<T>, maxConcurrent=5, per-key in-flight dedup) gates the Census's `rowStatsFor + notificationsForPatient` fetches. Visibility-lazy via `visibility_detector` proved fragile in release Flutter Web (intermittent callback dropouts post-service-worker handover), so Census eager-enqueues through the throttle as the safer default; `MaybeVisible` wrapper stays in the tree as defensive belt-and-suspenders. Sage-tinted skeleton placeholders render while rows load. **2B-FAC-R now substantively complete for V1 critical-path** — all five locked-in requirements (initial slice + L3+L11 polling + L6 alertType + L12 caching + L5 throttle) landed and live-validated against `pt_bench_98`. **Zero firmware-facing impact** — pure-portal performance work. Same day: §C31 TTL caching; §C30 L6 notification swap; §C29 PollingController; §C28.5 CR-1 deploy + validation. Next portal subset: 2B-FAC-W (facility writes — alert ack + Add/Edit/Discharge/Pause/CareNote)

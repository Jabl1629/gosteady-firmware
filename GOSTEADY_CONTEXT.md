...
- **Coord doc (append-only joint log):** `~/Documents/gosteady-portal/docs/firmware-coordination/2026-04-17-cloud-contracts.md` _(moved out of iCloud 2026-05-23 to fix tsc/npm slowness; iCloud copy at the old path may persist for a short confidence-window before deletion)_
  ...
  - **Latest entry: §C30 (2026-05-26)** — 2B-FAC-R L6 notification engine swap to server alertType: Census + Patient Detail now render server-authoritative rule names ("Battery critical", "No activity today", etc.) sourced from the existing `/alerts?status=unacknowledged` response, mapping each `alertType` via the L6 table. Demo build keeps the client-side three-rule engine; live build bypasses it. Live-validated against `pt_bench_98` — Census badge "Battery critical · 45", Patient Detail review panel renders 45 alerts with per-row mapped labels + relative-time subtitles. **Zero firmware-facing impact** — portal renders existing Alert History entries; no MQTT contract or cloud-Lambda change. Same day: §C29 PollingController landed; §C28.5 CR-1 heartbeat-processor lastSeen-mirror deploy + live validation. Prior milestones: §C27 (2026-05-24) three cloud deploys — 2A-0 amendment + 2A-UM-P + 1C-slim; §C24 (2026-05-18) connection-coordinator Lambda live

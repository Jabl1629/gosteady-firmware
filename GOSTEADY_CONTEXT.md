...
| `build_pilot/` | `prj_pilot.conf` (self-contained) | **Battery pilot image (0.13.0-pilot).** = field + snippets OFF + `CONFIG_GOSTEADY_LOW_POWER`. Build single-overlay (sysbuild only forwards one `EXTRA_CONF_FILE` cleanly): `-DEXTRA_CONF_FILE=prj_pilot.conf`. |

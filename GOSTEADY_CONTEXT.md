...
- **Coord doc (append-only joint log):** `~/Documents/gosteady-portal/docs/firmware-coordination/2026-04-17-cloud-contracts.md` _(moved out of iCloud 2026-05-23 to fix tsc/npm slowness; iCloud copy at the old path may persist for a short confidence-window before deletion)_
  ...
  - **Latest entry: §C31 (2026-05-26)** — 2B-FAC-R L12 30 s TTL caching + in-flight Future dedup landed in portal: new file-scope `_TimedCache<K,V>` helper in `LiveFacilityRepository` collapses concurrent same-key fetches into one HTTP request (closes the duplicate `?range=30d` fan-out surfaced in §C29.2) + serves consumer reads from cache within the 30 s window matching Patient Detail's polling cadence. **2B-FAC-R substantively complete for V1 critical-path** (initial slice + L3+L11 polling + L6 alertType + L12 caching all deployed); only L5 lazy-per-row throttle remains, deferred until a multi-patient pilot exposes API throttle pressure. **Zero firmware-facing impact** — pure-portal optimization. Same day: §C30 L6 notification engine swap; §C29 PollingController; §C28.5 CR-1 deploy + validation. Prior milestones: §C27 (2026-05-24) three cloud deploys — 2A-0 amendment + 2A-UM-P + 1C-slim; §C24 (2026-05-18) connection-coordinator Lambda live

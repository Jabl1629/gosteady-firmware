...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	event->dev_idx = 1; /* GoSteady M6b: NUS tunnels to uart1 (command channel), not uart0 (console) */
...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	event->peer_id = PEER_ID_BLE;
	event->dev_idx = 1; /* GoSteady M6b: NUS tunnels to uart1 */
	event->baudrate = 0; /* Don't care */
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	if (is_uart_data_event(aeh)) {
		...
		/* GoSteady M6b: single BLE Service instance mapped to UART_1.
		 * UART_0 is the nRF9151 app console (log output only; nothing
		 * listens there). UART_1 is where our M5/M6a command parser
		 * lives, so we pipe NUS traffic to it. Original upstream
		 * Nordic default was UART_0. */
		if (event->dev_idx != 1) {
			return false;
		}
